...
/// bo'lmasa kulrang, bo'lsa ko'k). Backend / Fiskal / Printer kartalari.
///
/// Terminal endigina o'rnatilganda bu ekranga login sahifasidan "000" kodi
/// bilan kiriladi (⚙️ tugmasi olib tashlangan). Base URL, terminal kodi va
/// boshqalar to'g'rilanib "Saqlash" bosilganda — sozlamalar saqlanadi va
...
class _SettingsScreenState extends ConsumerState<SettingsScreen> {
  ...
  Future<void> _save() async {
    await _persist();
    if (!mounted) return;
    // Ildizni xabardor qilamiz — birinchi o'rnatish (setup) bo'lsa login'ga o'tadi.
    ref.read(configVersionProvider.notifier).state++;
    setState(() {
      _dirty = false;
      _showToast = true;
    });
    // Mustaqil (login/setup) rejimda: toast ko'ringach ekran yopilib login
    // yangi sozlama bilan qaytadan ishlaydi. Embedded (bosh qobiq) rejimda
    // ekran yopilmaydi — shunchaki toast ko'rsatiladi.
    ...
    return Column(
      ...
      children: [
        ...
        Row(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            // Orqaga — sozlamalar alohida ochilganda (login yoki bosh
            // ekrandan) chiqish uchun. Tiqilib qolmaslik uchun doim ko'rinadi.
