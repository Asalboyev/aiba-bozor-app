...
/// chapda aurora fon + shisha logo, o'ngda qora panel (AIBA + PIN + klaviatura).
/// PIN-only: kassir 4 xonali PIN teradi → to'liq bo'lganda AVTOMATIK kiradi.
...
// Parol 3–4 raqam (kassa ilovasi bilan bir xil). 3 raqamli eski kodlar ✓/Enter
// bilan, 4-raqam terilishi bilan AVTOMATIK kiriladi.
const int _pinMin = 3;
const int _pinMax = 4;
...
class _LoginScreenState extends ConsumerState<LoginScreen> {
  ...
  void _digit(String d) {
    ...
    // Yangi raqam terila boshlaganda eski xato yo'qoladi.
    if (ref.read(loginControllerProvider).error != null) {
      ref.read(loginControllerProvider.notifier).clearError();
    }
    setState(() => _pin += d);
    if (_pin.length >= _pinMax) _submit();
    // Yashirin avariya kodi "000" — Sozlamalarni ochadi (login sahifasida
    // ko'rinadigan tugma yo'q). Odatda sozlash birinchi o'rnatishda bir marta
    // qilinadi; bu kod noto'g'ri sozlashdan keyin ham qayta kirish uchun.
    if (_pin == '000') {
      setState(() => _pin = '');
  ...
  Widget build(BuildContext context) {
    ...
    // Xato paydo bo'lsa — 20 soniyadan so'ng o'zi yo'qoladi (raqam terilsa ham
    // darhol ketadi). Har yangi xatoda taymer qayta boshlanadi.
...
/// Chap tomon — aurora fon + shisha logo (Figma'dan kesilgan bitta rasm).
...
/// O'ng paneldagi kichik app-icon — flat AIBA belgisi (SVG), yumaloq kvadratda.
    ...
    return Container(
      ...
      child: Stack(
        children: [
                ...
                child: SizedBox(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      // BOZOR ilovasi — kassa emas: nomi ham shunga mos.
                      const Text('AIBA Bozor',
                          style: TextStyle(color: Colors.white, fontSize: 40, fontWeight: FontWeight.w800, letterSpacing: .5)),
                      const SizedBox(height: 4),
                      const Text('Zakaz · Bozor · Qabul',
                          style: TextStyle(color: Color(0xFF9AA0A6), fontSize: 15)),
                      const SizedBox(height: 28),
                      const Text('Shaxsiy parolingizni kiriting',
                          style: TextStyle(color: Color(0xFF6B7178), fontSize: 13.5)),
                      ...
                      // Yuklanish holati — «Kirish» bosilgandan keyin.
                      SizedBox(
                        height: 24,
                        child: loading
                            ? const SizedBox(
          ...
          // Xato toasti — eng ustda (X bosilsin).
          if (error != null)
            Positioned(
              top: 12,
              left: 20,
  ...
  Widget build(BuildContext context) {
    ...
    return Row(
      ...
      children: List.generate(_pinMax, (i) {
        // Xato holatida barcha nuqtalar qizil to'la ko'rinadi.
