...
/// yumaloq panel ichida. Bosilsa savatga qo'shiladi.
...
Future<void> _submitSearch(BuildContext context, WidgetRef ref,
  ...
  num qty = 1;
  var term = raw;
  final m1 = RegExp(r'^(\d+(?:[.,]\d+)?)\s*[*xx]\s*(.+)$').firstMatch(raw);
  final m2 = RegExp(r'^(.+?)\s*[*xx]\s*(\d+(?:[.,]\d+)?)$').firstMatch(raw);
  if (m1 != null) {
                ...
                Expanded(
                  child: Stack(
                    children: [
                      ...
                      // Tor rejimda kod rasm ustida — nom qatoriga joy bo'shaydi.
...
class _ImagePlaceholder extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    // Rasm bo'lmaganda — neytral to'q plitka (Figmada vilka/ikon yo'q).
    return DecoratedBox(
