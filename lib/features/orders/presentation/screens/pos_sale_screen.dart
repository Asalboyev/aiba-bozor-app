...
/// Oxirgi "xato" deb belgilangan chek raqami — keyingi (to'g'ri) chekda
...
class _PosSaleScreenState extends ConsumerState<PosSaleScreen> {
  ...
  /// F9 — joriy zakazni bekor qilish. Savatda mahsulot bo'lsa: SABAB so'raladi
  ...
  Future<void> _closeOrder(BuildContext context, WidgetRef ref) async {
    ...
    // Xato chekdan keyin ham, oddiy bekor qilishda ham order DARHOL YOPILADI —
  ...
  Future<void> _registerAndPrintErrorCheck(BuildContext context, WidgetRef ref,
    ...
    // Keyingi (to'g'irlangan) chekda shu raqamga havola qoldiramiz.
  ...
  Future<void> _checkout(
    ...
    // QR bosilsa — Payme/Click/QR skaner oynasi; aks holda oddiy to'lov oynasi.
  ...
  Future<void> _autoPrintAfterFiscal(BuildContext context, WidgetRef ref,
    ...
    if (needPoll) {
      ...
      // F12 bilan qayta chop etilganda ham yangi QR chiqsin.
      _lastResult = r;
    }
    if (!context.mounted) return;
    try {
  ...
  /// XATO CHEK bannerli nusxa (soliq QRsiz) chop etiladi.
  Future<void> _printErrorCopy(
    ...
    final errCopy = ReceiptData(
      ...
      header: receipt.header,
      footer: receipt.footer,
      // Xato chekda soliq QR kerak emas — u hisobga olinmaydi.
      showQr: false,
      showMxik: receipt.showMxik,
      paperWidth: receipt.paperWidth,
      isErrorCheck: true,
